...
class PrecomputedCrackDataset(Dataset):
    ...
    Args:
        ...
        oversample_positive: If True and metadata.json exists, assign higher
                             sampling weight to patches that contain cracks.
        positive_weight:    Weight multiplier for crack-containing patches
                            (patches with crack_ratio > crack_threshold).
        crack_threshold:    Minimum crack_ratio to qualify as a positive patch.
    """
    ...
    def __init__(
        ...
        if oversample_positive:
            ...
            if metadata_path.exists():
                metadata: dict[str, float] = json.loads(
                    metadata_path.read_text(encoding="utf-8")
                )
                weights: list[float] = []
                n_positive = 0
                for rf in self._rgb_files:
                    key = rf.stem  # matches key written by precompute_patches.py
                    ratio = metadata.get(key, 0.0)
                ...
                print(
                    f"[dataset] oversample_positive: {n_positive}/{len(self._rgb_files)} "
                    f"positive patches (weight={positive_weight}x)"
                )
            else:
                print(
                    f"[dataset] oversample_positive=True but metadata.json not found at "
                    f"{metadata_path}. Falling back to uniform sampling.\n"
                    "  Run: python scripts/precompute_patches.py to generate metadata."
                )
    ...
    def get_sample_weights(self) -> list[float]:
        ...
        Returns uniform weights if metadata was unavailable."""
        if self._sample_weights is not None:
            return self._sample_weights
        return [1.0] * len(self._rgb_files)

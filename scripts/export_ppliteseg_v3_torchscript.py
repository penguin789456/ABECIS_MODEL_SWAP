"""
匯出 PP-LiteSeg-T v3（+SCD pseudo-label）checkpoint 為 TorchScript 模型

用途：
  將 best.pth 權重 + PP-LiteSeg 架構匯出成可獨立載入的 TorchScript .pt

執行方式：
  conda activate CrackSeg
  cd H:\ChihleeMaster\dev\ABECIS_MODEL_SWAP
  python export_ppliteseg_v3_torchscript.py
"""
...
# ── 路徑設定 ─────────────────────────────────────────────
CFG_PATH = Path(r"H:\ChihleeMaster\dev\ABECIS_MODEL_SWAP\configs\final\ppliteseg_v3.yaml")

CKPT_PATH = Path(
    r"H:\ChihleeMaster\dev\ABECIS_MODEL_SWAP\outputs\checkpoints\ppliteseg_v3\best.pth"
)

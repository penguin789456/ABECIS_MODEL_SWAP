"""
匯出 DeepLabV3-MobileNetV3 checkpoint 為 TorchScript 模型

用途：
  將 best.pth 權重 + DeepLabV3 架構匯出成可獨立載入的 TorchScript .pt

執行方式：
  conda activate CrackSeg
  cd H:\ChihleeMaster\dev\ABECIS_MODEL_SWAP
  python export_deeplabv3_torchscript.py
"""
...
# ── 路徑設定 ─────────────────────────────────────────────
CFG_PATH = Path(r"H:\ChihleeMaster\dev\ABECIS_MODEL_SWAP\configs\final\deeplabv3_mobilenet.yaml")

CKPT_PATH = Path(
    r"H:\ChihleeMaster\dev\ABECIS_MODEL_SWAP\outputs\checkpoints\deeplabv3_mobilenet\20260422_114901\best.pth"
)

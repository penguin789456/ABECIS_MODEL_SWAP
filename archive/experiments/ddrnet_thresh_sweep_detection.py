"""
DDRNet 閾值掃描：偵測率 vs Precision 完整曲線
使用與 inference_crackseg.py 完全相同的邏輯（平均 logit → sigmoid）
"""
import sys, os
...
# ── 載入模型（與 inference_crackseg.py 完全相同）─────────────────────────────
...
# ── 計算機率圖（複製自 stitch_patches，改回傳 prob 而非 binary）──────────────
def get_prob_map(image):
    ...
    return 1.0 / (1.0 + np.exp(-avg_logit))  # sigmoid → prob map
...
for i, img_id in enumerate(test_ids):
    ...
    # GT resized to match prob shape (NEAREST)
...
print(f'\n{"閾值":>6} {"偵測率":>8} {"漏檢率":>8} {"IoU":>8} {"Precision":>10} {"Recall":>8}')
print('-'*60)
...
for thresh in thresholds:
    ...
    print(f'{thresh:>6.2f} {dr:>8.1%} {1-dr:>8.1%} {iou:>8.4f} {prec:>10.4f} {rec:>8.4f}')

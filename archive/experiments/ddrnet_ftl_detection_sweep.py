"""
DDRNet FTL 偵測率掃描（只看連通分量是否被偵測到，不看精確度）
使用 FTL best.pth checkpoint
"""
import sys, os
...
CFG_PATH = r'H:\ChihleeMaster\dev\ABECIS_MODEL_SWAP\configs\final\ddrnet.yaml'
FTL_CKPT = r'H:\ChihleeMaster\dev\ABECIS_MODEL_SWAP\outputs\checkpoints\ddrnet_ftl\20260501_181112\best.pth'
MIN_PX   = 50
...
print(f'Checkpoint: {FTL_CKPT}')

# 載入 FTL 模型
model = build_model(cfg['model']).to(DEVICE)
ckpt = torch.load(FTL_CKPT, map_location=DEVICE)
model.load_state_dict(ckpt['model'] if 'model' in ckpt else ckpt)
model.eval()
print(f'Loaded FTL checkpoint (ep best)')
...
def get_prob_map(image):
    ...
    return 1.0 / (1.0 + np.exp(-avg_logit))
...
# 閾值掃描
thresholds = [0.05, 0.10, 0.20, 0.30, 0.40, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80]

print(f'\n{"閾值":>6} {"偵測率":>8} {"漏檢率":>8} {"IoU":>8} {"Precision":>10} {"Recall":>8}  {"超越ABECIS":>10}')
print('-'*72)
...
for thresh in thresholds:
    ...
    dr   = det_total / total_regions
    iou  = tp / (tp+fp+fn) if (tp+fp+fn) else 0
    prec = tp / (tp+fp)    if (tp+fp)    else 0
    rec  = tp / (tp+fn)    if (tp+fn)    else 0
    beat = '★ YES' if dr > ABECIS_DR else ''
    print(f'{thresh:>6.2f} {dr:>8.1%} {1-dr:>8.1%} {iou:>8.4f} {prec:>10.4f} {rec:>8.4f}  {beat}')

print(f'\nABECIS: 偵測率=80.4%  漏檢率=19.6%  IoU=0.3086  Prec=0.327  Rec=0.846')

...
- **待完成**：手動執行訓練（見下方「訓練教學」）
- **已修正**（2026-03-27）：
  - DeepLabV3-MobileNetV3 `in_channels` 從 backbone 動態讀取（commit `fc383e0`）
...
| 模型 | 框架 | Test IoU | FPS（待填） | 參數量 |
|------|------|----------|------------|--------|
| Mask R-CNN（ABECIS 基準） | Instance Seg | 訓練中 | ~3 | 44M |
| DeepLabV3 (MobileNetV3-Large) | Semantic Seg | 訓練中 | ~30 | 11M |
| DDRNet-23-slim | Semantic Seg | **0.3912** | ~40 | 5.6M |
| PP-LiteSeg-T | Semantic Seg | **0.4391** | 30+ | 5M |
...
> 在這個統一標準下，DeepLabV3-MobileNetV3 達到 __，精度相當的情況下推論速度提升約 __ 倍；PP-LiteSeg 以 __ 倍速度換取約 __% 的精度犧牲，適合邊緣裝置即時部署。」
...
```bash
# 1. DeepLabV3-MobileNetV3（語意分割基準）
...
```
outputs/
├── checkpoints/{deeplabv3_mobilenet,ppliteseg,pidnet,maskrcnn}/
...
| 錯誤訊息 | 解法 |
|---------|------|
| `CUDA out of memory` | 降低 batch_size：`configs/deeplabv3_mobilenet.yaml` → `batch_size: 4` |
...
| `CrackSeg` | PP-LiteSeg、PIDNet、DeepLabV3-MobileNetV3 | 新建，見下方 yaml |

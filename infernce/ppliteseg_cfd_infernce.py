"""
PP-LiteSeg-T TorchScript Whole-image 推論腳本 — CFD 資料集

功能：
  1. 載入 TorchScript 模型：ppliteseg_torchscript.pt
...
啟動方式：
  ...
  python "H:\ChihleeMaster\dev\final_outputs\infernce\ppliteseg_cfd_infernce.py" --device cuda
  python "H:\ChihleeMaster\dev\final_outputs\infernce\ppliteseg_cfd_infernce.py" --device cpu

輸出：
  H:\ChihleeMaster\dev\final_outputs\
    logs\ppliteseg_cfd\{gpu|cpu}\inference_log_{timestamp}.txt
    logs\ppliteseg_cfd\{gpu|cpu}\per_image_{timestamp}.csv
    before\ppliteseg_cfd\{gpu|cpu}\*.png
    after\ppliteseg_cfd\{gpu|cpu}\*.png
    pred_masks\ppliteseg_cfd\{gpu|cpu}\*.png
"""
...
# ── 路徑設定 ────────────────────────────────────────────────────────────────
MODEL_PATH = Path(
    r"H:\ChihleeMaster\dev\ABECIS_MODEL_SWAP\outputs\checkpoints\ppliteseg\ppliteseg_torchscript.pt"
)

# CFD 完全未見資料集
RGB_DIR = Path(r"H:\ChihleeMaster\CrackK500\CFD\cfd_image")
OUT_BASE = Path(r"H:\ChihleeMaster\dev\final_outputs")
MODEL_NAME = "ppliteseg_cfd"
...
def main():
    ...
    log_lines = [
        f"PP-LiteSeg-T TorchScript Whole-image 推論記錄 — CFD",

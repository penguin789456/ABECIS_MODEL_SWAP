# -*- coding: utf-8 -*-
"""
CFD（Crack Forest Dataset）— 512×512 Patch 滑動視窗推論腳本（TorchScript 通用版）

與訓練條件一致：patch_size=512, overlap=128（stride=384）
CFD 影像尺寸約 640×480，每張產生 1~4 個 patch，與 Mendeley 管線完全對齊。

用法：
  conda activate CrackSeg
  python cfd_patch_inference.py --model ppliteseg  --device cuda
  python cfd_patch_inference.py --model ddrnet      --device cuda
  python cfd_patch_inference.py --model ddrnet_ftl  --device cuda
  python cfd_patch_inference.py --model deeplabv3   --device cuda

輸出（不覆蓋全圖結果）：
  pred_masks/{model}_cfd_patch/{device}/
  logs/{model}_cfd_patch/{device}/inference_log_{ts}.txt
"""
...
def main():
    ...
    exts = {".jpg", ".jpeg", ".png"}
    image_paths = sorted(
        [(p.stem, p) for p in RGB_DIR.iterdir() if p.suffix.lower() in exts],
        key=lambda x: x[0]
    )
    ...
    print(f"資料集    : {RGB_DIR}")
    print(f"影像數    : {n_images} 張（全部 CFD cfd_image）")
    ...
    lines = [
        f"CFD Patch 推論記錄 — {out_name}",
        f"時間        : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"模型        : {pt_path}",
        f"資料來源    : {RGB_DIR}（全部 {n_images} 張）",

...
啟動方式：
  ...
  python run_ddrnet_torchscript_whole_csv_resource.py --device cpu

GPU：
  python run_ddrnet_torchscript_whole_csv_resource.py --device cuda

指定輸入尺寸與閾值：
  python run_ddrnet_torchscript_whole_csv_resource.py --target-size 1024 1024 --threshold 0.8 --device cpu
"""
...
def main():
    ...
    log_path = log_dir / f"{MODEL_NAME}_{device_suffix}_log.txt"
    csv_path = log_dir / f"{MODEL_NAME}_{device_suffix}_per_image.csv"
